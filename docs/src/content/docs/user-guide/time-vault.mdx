...
The time vault is a catalogue of deleted entities. Nothing is permanently lost; any deleted entity can be recovered from here.
...
-   **Sorting**: Items are ordered by **Deletion Time**, so the most recently deleted resources come first. You can reverse the direction to start from the oldest.
...
-   **Restore**: This button restores the entity to your collection and removes it from the time vault.