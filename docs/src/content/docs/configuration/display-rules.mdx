...
The display rules file is the heart of HERITRACE's user interface customization. It controls how data is presented and handled without touching a single line of code: which types of entities are displayed, what properties are visible, and how forms behave for creating and editing data.
...
The application reads display rules from `/app/display_rules.yaml` inside the container. To use your own rules, mount the file as a Docker volume:
...
-   `fetchUriDisplay`: A SPARQL query that creates a user-friendly label for an entity, replacing technical URIs with meaningful text. See the [Displaying entity labels](#displaying-entity-labels-with-fetchuridisplay) section for details.
...
The query uses `[[uri]]` as a placeholder, which HERITRACE replaces with the URI of the entity being displayed.

The query must return `?display`. If it fails or returns no result, HERITRACE reports the error.

You can find many practical examples of this in the default `display_rules.yaml` file.
...
<Aside type="caution" title="Queries that follow incoming references">
A query can also bind `[[uri]]` as an object, building the label from the entities that point at it. HERITRACE detects this and loads those entities on history pages, which is expensive when an entity has many referrers, so prefer following references away from the entity. In fact, such a label resolves only for snapshots taken after the referring entity existed.
</Aside>
...
HERITRACE queries the triplestore to find other entities of the same type that share matching values for the configured properties. The configuration supports boolean logic combining OR and AND conditions.
...
When running HERITRACE in development mode, any changes to `display_rules.yaml` will automatically restart the server, so you can see your updates instantly.
