...
The application reads the SHACL schema from `/app/shacl.ttl` inside the container. To use your own schema, mount it as a Docker volume:
...
Example configurations are available in `example_configurations/`.
...
Control how many values a property can have. Both count every value of `sh:path` on the
entity, across all the property constraints declared for it. To constrain the number of
occurrences of one specific value rather than the total, see
[Occurrences of a specific value](#occurrences-of-a-specific-value-shqualifiedvalueshape).
...
HERITRACE renders a property constrained with `sh:hasValue` as a read-only field: the value is
displayed, marked as mandatory, and cannot be edited, added, or removed.

### Occurrences of a specific value (`sh:qualifiedValueShape`)
Constrain how many values of a property match a given condition, leaving the other values
unconstrained.

- `sh:qualifiedValueShape`: The condition that the counted values must satisfy.
- `sh:qualifiedMinCount` and `sh:qualifiedMaxCount`: How many values may satisfy it.

Use this instead of `sh:hasValue` with `sh:minCount`/`sh:maxCount` whenever the property carries
several values at once. A common case is an entity typed with both a general and a specific
class: `sh:maxCount 1` would count both types and could never be satisfied, whereas the
qualified counts apply to each type separately.

```ttl
# The entity must be typed both fabio:Expression and fabio:JournalArticle
sh:property [
    sh:path rdf:type ;
    sh:qualifiedValueShape [ sh:hasValue fabio:Expression ] ;
    sh:qualifiedMinCount 1 ;
    sh:qualifiedMaxCount 1 ;
] ;
sh:property [
    sh:path rdf:type ;
    sh:qualifiedValueShape [ sh:hasValue fabio:JournalArticle ] ;
    sh:qualifiedMinCount 1 ;
    sh:qualifiedMaxCount 1 ;
] .
```

Nesting `sh:or` inside the qualified shape turns the constraint into a choice among values,
which HERITRACE renders as a dropdown:

```ttl
# At most one further type, picked from the listed ones
sh:property [
    sh:path rdf:type ;
    sh:qualifiedValueShape [
      sh:or (
        [ sh:hasValue fabio:Book ]
        [ sh:hasValue fabio:Thesis ]
      )
    ] ;
    sh:qualifiedMaxCount 1 ;
] .
...
A single property constraint must not declare both plain and qualified counts: HERITRACE would
read them as two separate fields on the same property.
...
SHACL and Display Rules are both individually optional, but their interaction follows specific rules that determine data validation, form generation, and user interface behavior.
...
- **Without SHACL**: Users get a custom properties form where they can define any property URI and value.
...
- **Update display rules**: If you modify properties, remember to update `display_rules.yaml` to reflect those changes in the user interface.
