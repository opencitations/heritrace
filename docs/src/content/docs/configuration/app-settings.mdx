...
HERITRACE supports two configuration methods: a Python configuration file (`config.py`) for development, and Docker environment variables for containerized deployments. This page covers all available configuration options.

## Configuration Methods

HERITRACE can be configured using either:

1. **Python Configuration File** (`config.py`) - Traditional method, ideal for development
2. **Docker Environment Variables** - Modern approach, recommended for production and Docker deployments

<Tabs>
  <TabItem label="Docker Environment Variables (Recommended)">
    When using Docker (as shown in the [Quick Start guide](/heritrace/getting-started/quick-start/)), configuration is handled through environment variables in your `docker-compose.yml` file:
  ...
  <TabItem label="Python Configuration File">
    ### Initial Setup
...
<Aside type="tip" title="Configuration Priority">
When using Docker, environment variables take precedence over `config.py` settings. This allows you to override specific settings without modifying the configuration file.
</Aside>

<Aside type="tip" title="Configuration Management">
Keep sensitive information (like ORCID credentials and secret keys) out of version control. Use environment variables for production deployments.
</Aside>
