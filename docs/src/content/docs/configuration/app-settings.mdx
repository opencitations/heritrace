...
| `MAX_WORKERS` | Integer | Maximum number of auxiliary worker processes available to HERITRACE | **Yes** | None |
...
Both development and production environments use Gunicorn as the WSGI server. The number of workers and request timeout are configurable.
...
|---------------------|------|-------------|---------|
| `GUNICORN_WORKERS` | Integer | Number of Gunicorn worker processes | `(2 * CPU cores) + 1` |
| `GUNICORN_TIMEOUT` | Integer | Maximum seconds a worker can spend handling a single request before being restarted | `120` |

In production, Gunicorn serves plain HTTP. Use a reverse proxy (e.g., nginx) to terminate HTTPS. In development, Gunicorn serves HTTPS directly using a self-signed certificate generated on first startup.
...
For managing your Virtuoso instance (launching the server, bulk-loading data, exporting dumps, rebuilding the full-text index), see <a href="https://github.com/opencitations/virtuoso_utilities" target="_blank" rel="noopener noreferrer">**virtuoso-utilities**</a>. This Python package automates tasks that are otherwise tedious to configure in Virtuoso.
</Aside>
...
The application reads SHACL and display rules from fixed paths inside the container: `/app/shacl.ttl` and `/app/display_rules.yaml`. These are not configurable via environment variables. To customize them, mount your files as Docker volumes:

```yaml
volumes:
  - ./my_shacl.ttl:/app/shacl.ttl
  - ./my_display_rules.yaml:/app/display_rules.yaml
```
...
|---------------------|-------------|---------|
| `PRIMARY_SOURCE` | Defines the DOI or URL recorded as `prov:hadPrimarySource` for the initial historical snapshot of the pre-existing dataset | Required |
...
## Optional save plugin

`SAVE_PLUGIN_CLASS` selects a component that receives the working RDF graph after
the dataset and provenance SPARQL updates and before HERITRACE commits its internal
changes. `SAVE_PLUGIN_OPTIONS` accepts a JSON object whose properties are passed to
the component constructor. The component must provide:
...
Leaving both variables unset disables the extension.

The included OpenCitations Meta component updates the zipped JSON-LD data and
provenance files using the Meta directory convention. It requires
`DATASET_IS_QUADSTORE=true`.

```yaml
environment:
  SAVE_PLUGIN_CLASS: default_components.meta_rdf_file_writer.MetaRDFFileWriter
  SAVE_PLUGIN_OPTIONS: '{"rdf_dir":"/app/rdf","base_iri":"https://w3id.org/oc/meta/","dir_split_number":10000,"items_per_file":1000}'
volumes:
  - ./rdf:/app/rdf
```

The component rewrites only archives containing changed entities. It locks each
archive during read-modify-write operations so multiple application workers do
...
### Built-in default (no code)

Leaving both `URI_GENERATOR_CLASS` and `COUNTER_HANDLER_CLASS` unset enables the built-in default: a UUID-based generator that mints IRIs of the form `BASE_IRI/<uuid>`, with provenance snapshot counters stored automatically in the Redis instance at `REDIS_URL`. This requires no code, only the `BASE_IRI` environment variable.

```yaml
environment:
  - BASE_IRI=https://example.org/my-collection
  # URI_GENERATOR_CLASS and COUNTER_HANDLER_CLASS left unset
```

### Optional and custom components

Setting `URI_GENERATOR_CLASS` replaces the built-in UUID generator. Set
`COUNTER_HANDLER_CLASS` as well when the selected generator uses counters.

`COUNTER_HANDLER_OPTIONS` and `URI_GENERATOR_OPTIONS` accept JSON objects. HERITRACE
passes their properties as keyword arguments to the selected constructors. For
example:

```yaml
environment:
  COUNTER_HANDLER_CLASS: custom_components.my_counter.FileCounterHandler
  COUNTER_HANDLER_OPTIONS: '{"directory":"/app/counters"}'
  URI_GENERATOR_CLASS: custom_components.my_uri.SequentialURIGenerator
  URI_GENERATOR_OPTIONS: '{"base_iri":"https://example.org"}'
```
...
<Aside type="caution">
`COUNTER_HANDLER_CLASS` is only required if your `URI_GENERATOR` uses a counter-based strategy. If you use a non-counter generator (like the UUID example above), you can omit it: HERITRACE then provides a generic Redis-backed counter handler automatically, since provenance snapshot numbering always relies on a counter handler.
</Aside>

The `COUNTER_HANDLER` is a component designed to work with **counter-based** URI generators. It manages the persistent state of the numerical counters. Without it, counter-based URI generators cannot guarantee uniqueness across application restarts.
...
5.  **Safelist Your ORCID ID**:
    To enable access, you must add authorized ORCID IDs to the `ORCID_SAFELIST`. The application automatically extracts the ID from full ORCID URLs, so you can use either format.
...
<Tabs>
  ...
  <TabItem label="Proxy Handling">
    ...
    A property is treated as a proxy relationship in `display_rules.yaml` by using the `intermediateRelation` key. This key tells HERITRACE to create an intermediate entity to connect the subject to the target object.
...
`COUNT_LIMIT` optimizes catalog queries for large datasets. When counting entities of a specific class, the query stops at `COUNT_LIMIT + 1`. If the limit is exceeded, the catalog displays "LIMIT+" (e.g., "10000+") instead of the exact count, preventing timeouts on very large result sets.
</Aside>
