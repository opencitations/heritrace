...
def test_counter_handler_basic_operations() -> None:
    ...
    assert updated_counter_value == new_value, (
        "Counter should be incremented to the new value"
    )


def test_counter_handler_with_different_entities() -> None:
    ...
    for i, entity_name in enumerate(entity_names):
        ...
        assert counter_handler.read_counter(entity_name) == value, (
    ...
    for i, entity_name in enumerate(entity_names):
        ...
        assert counter_handler.read_counter(entity_name) == expected_value, (
            f"Counter for {entity_name} should still be {expected_value}"
        )


def test_counter_handler_negative_value() -> None:
    ...
    assert counter_handler.read_counter(entity_name) == 0, (
