...
from heritrace.utils.shacl_validation import get_valid_predicates, validate_new_triple


class TestShaclValidation(unittest.TestCase):
    """Test class for shacl_validation module."""

    def test_validate_new_triple_matching_triples(self) -> None:
        """
        ...
        with patch(
            ...
            with (
                ...
                assert returned_old_value == existing_value
                assert str(returned_old_value) == "Existing Value"
                assert isinstance(returned_old_value, Literal)
                assert returned_old_value.datatype == XSD.string

                assert isinstance(valid_value, Literal)
                assert str(valid_value) == "New Value"

                assert error == ""

    def test_get_valid_predicates_no_shacl(self) -> None:
        ...
        with patch(
            "heritrace.utils.shacl_validation.get_shacl_graph", return_value=None
        ):
            ...
            assert len(can_add) == 3
            assert str(RDF.type) in can_add
            assert str(predicate1) in can_add
            assert str(predicate2) in can_add
            # Verify all items in can_add are strings
            assert all(isinstance(item, str) for item in can_add), (
                "can_add should contain only strings,"
                f" got: {[type(item) for item in can_add]}"
            )

            assert len(datatypes) == 3
            for pred in [RDF.type, predicate1, predicate2]:
                assert str(pred) in datatypes
                assert datatypes[str(pred)] == XSD.string

            assert mandatory_values == {}
            assert optional_values == {}

            assert len(can_delete) == 3
            # Verify all items in can_delete are strings
            assert all(isinstance(item, str) for item in can_delete), (
                "can_delete should contain only strings,"
                f" got: {[type(item) for item in can_delete]}"
            )
            expected_predicates = {str(RDF.type), str(predicate1), str(predicate2)}
            assert set(can_delete) == expected_predicates
            assert len(all_predicates) == 3

    def test_get_valid_predicates_type_consistency(self) -> None:
        """Test that get_valid_predicates returns consistent string types regardless of
        SHACL presence.
        ...
        with patch(
            ...
            can_add_no_shacl, can_delete_no_shacl = (
                result_no_shacl[0],
                result_no_shacl[1],
            )
        ...
        empty_graph = Graph()
        with patch(
            "heritrace.utils.shacl_validation.get_shacl_graph", return_value=empty_graph
        ):
            result_empty_shacl = get_valid_predicates(triples, s_type)
            can_add_empty_shacl, can_delete_empty_shacl = (
                result_empty_shacl[0],
                result_empty_shacl[1],
            )
        ...
        for case_name, can_add, can_delete in [
            ...
            with self.subTest(case=case_name):
                assert all(isinstance(item, str) for item in can_add), (
                    f"case {case_name}: can_add should"
                    " contain only strings,"
                    f" got: {[type(item) for item in can_add]}"
                )
                assert all(isinstance(item, str) for item in can_delete), (
                    f"case {case_name}: can_delete should"
                    " contain only strings,"
                    f" got: {[type(item) for item in can_delete]}"
                )

    def test_determine_shape_for_classes_no_shacl(self) -> None:
        """
