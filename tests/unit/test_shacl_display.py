...
class TestShaclDisplay:
    ...
    def test_unordered_properties_are_preserved_exact_match(self) -> None:
        """
        Test that properties not in ordered_properties are included in the result with
        exact match (class and shape).
        """
        ...
        display_rules = [
            {
                "target": {"class": entity_class, "shape": entity_shape},
                "displayProperties": [{"property": "http://example.org/name"}],
            }
        ]
        ...
        assert set(ordered_props[1:]) == {
            "http://example.org/unordered",
            "http://example.org/another",
        }

        # Verify the content is preserved
        assert (
            result[entity_key]["http://example.org/name"]
            == form_fields[entity_key]["http://example.org/name"]
        )
        assert (
            result[entity_key]["http://example.org/unordered"]
            == form_fields[entity_key]["http://example.org/unordered"]
        )
        assert (
            result[entity_key]["http://example.org/another"]
            == form_fields[entity_key]["http://example.org/another"]
        )

    def test_unordered_properties_are_preserved_class_only(self) -> None:
        """
        Test that properties not in ordered_properties are included in the result with
        class-only match.
        """
        ...
        assert (
            result[entity_key1]["http://example.org/unordered"]
            == form_fields[entity_key1]["http://example.org/unordered"]
        )
        assert (
            result[entity_key2]["http://example.org/phone"]
            == form_fields[entity_key2]["http://example.org/phone"]
        )

    def test_unordered_properties_are_preserved_shape_only(self) -> None:
        """
        Test that properties not in ordered_properties are included in the result with
        shape-only match.
        """
        ...
        # Verify the content is preserved for unordered properties
        assert (
            result[entity_key1]["http://example.org/address"]
            == form_fields[entity_key1]["http://example.org/address"]
        )
        assert (
            result[entity_key2]["http://example.org/taxCode"]
            == form_fields[entity_key2]["http://example.org/taxCode"]
        )
