...
class TestValidateEntityData:
    ...
    def test_validate_entity_data_valid(self) -> None:
    ...
    def test_validate_entity_data_no_properties(self) -> None:
        """Test with no properties."""
        data = {"entity_type": "http://example.org/Person"}
        result = _validate_entity_data(data)
        assert result is None

    def test_validate_entity_data_empty_properties(self) -> None:
        """Test with empty properties."""
        data = {"properties": {}, "entity_type": "http://example.org/Person"}
        result = _validate_entity_data(data)
        assert result is None

    def test_validate_entity_data_no_entity_type(self) -> None:
        """Test with no entity_type."""
        data = {"properties": {"prop1": "value1"}}
        result = _validate_entity_data(data)
        assert result is None

    def test_validate_entity_data_empty_entity_type(self) -> None:
        """Test with empty entity_type."""
        data = {"properties": {"prop1": "value1"}, "entity_type": ""}
        result = _validate_entity_data(data)
        assert result is None
...
class TestGetVirtualPropertyConfigs:
    ...
    def test_get_virtual_property_configs_with_virtual_properties(
        ...
        mock_find_matching_rule.return_value = {
            "displayProperties": [
                {"displayName": "Virtual Prop", "isVirtual": True, "implementedVia": {}}
            ]
        }
    ...
    def test_get_virtual_property_configs_no_matching_rule(
        ...
        mock_get_display_rules.return_value = [
            {"target": {"class": "http://example.org/Other"}}
        ]
    ...
    def test_get_virtual_property_configs_without_virtual_flag(
        ...
        mock_find_matching_rule.return_value = {
            "displayProperties": [{"displayName": "Regular Prop", "isVirtual": False}]
        }
    ...
    def test_get_virtual_property_configs_without_display_name(
        ...
        mock_find_matching_rule.return_value = {
            "displayProperties": [{"isVirtual": True, "implementedVia": {}}]
        }
...
class TestGetVirtualPropertiesForEntity:
    ...
    def test_get_virtual_properties_for_entity(self, mock_get_configs) -> None:
    ...
    @patch("heritrace.utils.virtual_properties._get_virtual_property_configs")
    def test_get_virtual_properties_for_entity_no_configs(
        self, mock_get_configs
    ) -> None:
...
class TestApplyFieldOverrides:
    ...
    def test_apply_field_overrides_should_be_displayed(self) -> None:
        ...
        field_overrides = {"http://example.org/prop1": {"shouldBeDisplayed": False}}

        result = apply_field_overrides(form_field_data, field_overrides)

        assert "http://example.org/prop1" not in result

    def test_apply_field_overrides_display_name(self) -> None:
        """Test applying displayName override."""
        form_field_data = {"http://example.org/prop1": [{"displayName": "Old Name"}]}
        field_overrides = {"http://example.org/prop1": {"displayName": "New Name"}}

        result = apply_field_overrides(form_field_data, field_overrides)

        assert result["http://example.org/prop1"][0]["displayName"] == "New Name"

    def test_apply_field_overrides_value(self) -> None:
        ...
        field_overrides = {"http://example.org/prop1": {"value": "new_value"}}
    ...
    def test_apply_field_overrides_current_entity_placeholder(self) -> None:
        """Test applying ${currentEntity} placeholder."""
        form_field_data = {"http://example.org/prop1": [{"hasValue": "old_value"}]}
        field_overrides = {"http://example.org/prop1": {"value": "${currentEntity}"}}
        current_entity_uri = "http://example.org/entity123"

        result = apply_field_overrides(
            form_field_data, field_overrides, current_entity_uri
        )

        assert result["http://example.org/prop1"][0]["hasValue"] == current_entity_uri

    def test_apply_field_overrides_no_overrides(self) -> None:
        """Test when property has no overrides."""
        form_field_data = {"http://example.org/prop1": [{"displayName": "Prop 1"}]}
        field_overrides = {}
    ...
    def test_apply_field_overrides_multiple_details(self) -> None:
        ...
        assert all(
            d["displayName"] == "New Name" for d in result["http://example.org/prop1"]
        )

    def test_apply_field_overrides_filters_invisible_fields(self) -> None:
...
class TestTransformChangesWithVirtualProperties:
    ...
    @patch(
        "heritrace.utils.virtual_properties.process_virtual_properties_in_create_data"
    )
    def test_transform_changes_create_action(self, mock_process) -> None:
    ...
    def test_transform_changes_create_action_no_properties_remaining(
        self, mock_process
    ) -> None:
    ...
    def test_transform_changes_delete_virtual_property(self, mock_transform) -> None:
    ...
    @patch("heritrace.utils.virtual_properties.transform_virtual_property_deletion")
    def test_transform_changes_delete_virtual_property_returns_none(
        self, mock_transform
    ) -> None:
    ...
    def test_transform_changes_other_actions(self) -> None:
...
class TestProcessVirtualPropertiesInCreateData:
    ...
    def test_process_virtual_properties_in_create_data(
        ...
        mock_get_configs.return_value = {
            "Virtual Prop": {"isVirtual": True, "implementedVia": {}}
        }
        ...
        data = {
            "entity_type": "http://example.org/Person",
            "properties": {"name": "John", "Virtual Prop": [{"value": "test"}]},
        }
    ...
    @patch("heritrace.utils.virtual_properties._validate_entity_data")
    def test_process_virtual_properties_invalid_data(self, mock_validate) -> None:
    ...
    @patch("heritrace.utils.virtual_properties._validate_entity_data")
    @patch("heritrace.utils.virtual_properties._get_virtual_property_configs")
    def test_process_virtual_properties_no_configs(
        self, mock_get_configs, mock_validate
    ) -> None:
...
class TestTransformEntityCreationWithVirtualProperties:
    ...
    def test_transform_entity_creation(
        ...
        mock_validate.return_value = "http://example.org/Person"
        mock_get_configs.return_value = {"Virtual Prop": {"isVirtual": True}}
        mock_process_values.return_value = [
            {"entity_type": "VirtualEntity", "properties": {}}
        ]
        ...
        structured_data = {
            "entity_type": "http://example.org/Person",
            "properties": {"Virtual Prop": [{"value": "test"}]},
        }
        created_entity_uri = "http://example.org/entity123"

        result = transform_entity_creation_with_virtual_properties(
            structured_data, created_entity_uri
        )

        assert len(result) == 1

    @patch("heritrace.utils.virtual_properties._validate_entity_data")
    def test_transform_entity_creation_invalid_data(self, mock_validate) -> None:
    ...
    @patch("heritrace.utils.virtual_properties._validate_entity_data")
    @patch("heritrace.utils.virtual_properties._get_virtual_property_configs")
    def test_transform_entity_creation_no_configs(
        self, mock_get_configs, mock_validate
    ) -> None:
        """Test when there are no virtual property configs."""
        mock_validate.return_value = "http://example.org/Person"
...
class TestRemoveVirtualPropertiesFromCreationData:
    ...
    @patch("heritrace.utils.virtual_properties._validate_entity_data")
    @patch("heritrace.utils.virtual_properties._get_virtual_property_configs")
    def test_remove_virtual_properties(self, mock_get_configs, mock_validate) -> None:
    ...
    @patch("heritrace.utils.virtual_properties._validate_entity_data")
    def test_remove_virtual_properties_invalid_data(self, mock_validate) -> None:
    ...
    @patch("heritrace.utils.virtual_properties._validate_entity_data")
    @patch("heritrace.utils.virtual_properties._get_virtual_property_configs")
    def test_remove_virtual_properties_no_configs(
        self, mock_get_configs, mock_validate
    ) -> None:
        """Test when there are no virtual property configs."""
        mock_validate.return_value = "http://example.org/Person"
...
class TestTransformVirtualPropertyDeletion:
    ...
    def test_transform_virtual_property_deletion(self) -> None:
    ...
    def test_transform_virtual_property_deletion_not_virtual(self) -> None:
    ...
    def test_transform_virtual_property_deletion_no_flag(self) -> None:
        """Test when is_virtual flag is missing."""
        change = {"action": "delete", "object": "http://example.org/entity123"}

        result = transform_virtual_property_deletion(change)

        assert result is None

    def test_transform_virtual_property_deletion_no_object(self) -> None:
        """Test when object is missing."""
        change = {"action": "delete", "is_virtual": True}
...
class TestProcessVirtualPropertyValues:
    ...
    def test_process_virtual_property_values(self) -> None:
        """Test processing virtual property values."""
        values = [
            {"properties": {"name": "Test"}, "entity_shape": "http://example.org/Shape"}
        ]
        config = {
            "implementedVia": {
                "target": {
    ...
    def test_process_virtual_property_values_with_field_overrides(self) -> None:
        """Test processing with field overrides."""
        values = [{"properties": {}}]
        config = {
            "implementedVia": {
                "target": {"class": "http://example.org/IntermediateEntity"},
                "fieldOverrides": {
    ...
    def test_process_virtual_property_values_override_without_current_entity(
        self,
    ) -> None:
    ...
    def test_process_virtual_property_values_no_target_class(self) -> None:
        """Test when target has no class or shape."""
        values = [{"properties": {}}]
        config = {"implementedVia": {"target": {}}}

        result = process_virtual_property_values(values, config)

        assert result == []

    def test_process_virtual_property_values_non_dict_value(self) -> None:
    ...
    def test_process_virtual_property_values_override_displayed_true(self) -> None:
