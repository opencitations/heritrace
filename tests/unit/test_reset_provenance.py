...
def resetter(mock_counter_handler, mock_sparql, mock_logger):
    ...
    _, _mock_sparql_instance = mock_sparql
...
def test_init(resetter, mock_counter_handler, mock_sparql) -> None:
...
def test_get_entity_snapshots(resetter, mock_sparql) -> None:
    ...
    snapshots = resetter.get_entity_snapshots(entity_uri)
...
def test_delete_snapshots_empty(resetter, mock_sparql) -> None:
    """Test _delete_snapshots method with empty list."""
    _, mock_sparql_instance = mock_sparql

    result = resetter.delete_snapshots([])

    assert result is True
    mock_sparql_instance.setQuery.assert_not_called()


def test_delete_snapshots_success(resetter, mock_sparql) -> None:
    ...
    snapshots = [
        ...
        {
            "uri": "http://example.org/snapshot/2",
            "generation_time": "2023-01-02T00:00:00Z",
        },
    ]
...
def test_delete_snapshots_error(resetter, mock_sparql, mock_logger) -> None:
...
def test_reset_provenance_counter(resetter, mock_counter_handler, mock_logger) -> None:
    ...
    resetter.reset_provenance_counter(entity_uri)
...
def test_reset_entity_provenance_no_snapshots(
    _mock_convert, resetter, mock_sparql, mock_logger
) -> None:
...
@patch("heritrace.scripts.reset_provenance.convert_to_datetime")
def test_reset_entity_provenance_single_snapshot(
    mock_convert, resetter, mock_sparql, mock_logger, mock_counter_handler
) -> None:
    ...
    assert any(
        "removed invalidatedAtTime" in call[0][0]
...
@patch("heritrace.scripts.reset_provenance.convert_to_datetime")
def test_reset_entity_provenance_multiple_snapshots(
    mock_convert, resetter, mock_sparql, mock_logger, mock_counter_handler
) -> None:
...
@patch("heritrace.scripts.reset_provenance.convert_to_datetime")
def test_reset_entity_provenance_delete_failure(
    mock_convert, resetter, mock_sparql, mock_counter_handler
) -> None:
...
def test_reset_entity_provenance_string_uri(resetter, mock_sparql) -> None:
...
def test_reset_entity_provenance_function(mock_resetter_class) -> None:
    ...
    mock_resetter_class.assert_called_once_with(
        provenance_endpoint=provenance_endpoint, counter_handler=mock_counter_handler
    )
...
def test_load_config_success(mock_importlib_util) -> None:
...
def test_main_missing_config_class(
    ...
    def mock_hasattr_func(obj, name) -> bool:
        return not (obj == mock_config and name == "Config")
...
def test_main_missing_provenance_endpoint(
    ...
    def mock_hasattr_func(obj, name) -> bool:
        return not (obj == mock_config_class and name == "PROVENANCE_DB_URL")
...
def test_main_missing_counter_handler(
    ...
    def mock_hasattr_func(obj, name) -> bool:
        return not (obj == mock_config_class and name == "COUNTER_HANDLER")
...
def test_remove_invalidated_time_error(resetter, mock_sparql, mock_logger) -> None:
    ...
    result = resetter.remove_invalidated_time(snapshot)
...
def test_remove_invalidated_time_success(resetter, mock_sparql, mock_logger) -> None:
