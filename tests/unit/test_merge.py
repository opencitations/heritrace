...
MOCK_ENTITY1_PROPS = {
    "http://example.org/prop1": [
        {"value": "Value 1", "type": "literal", "readable_label": "Value 1"}
    ],
    "http://example.org/prop2": [
        {
            "value": "http://example.org/obj1",
            "type": "uri",
            "readable_label": "Object One",
        }
    ],
}
MOCK_ENTITY2_PROPS = {
    "http://example.org/prop3": [
        {"value": "Value 3", "type": "literal", "readable_label": "Value 3"}
    ],
}
...
def test_get_entity_details_success(
    mock_get_entity_types, mock_get_custom_filter, mock_get_sparql, app
) -> None:
    ...
    mock_get_entity_types.side_effect = [entity1_types, obj_type]
    ...
    mock_custom_filter_instance.human_readable_entity.assert_called_once_with(
        obj_uri, (obj_type[0], None)
    )


@patch("heritrace.routes.merge.get_sparql")
@patch("heritrace.routes.merge.get_entity_types", return_value=[])
def test_get_entity_details_no_types(
    mock_get_entity_types, mock_get_sparql, app
) -> None:
...
@patch("heritrace.routes.merge.get_sparql")
@patch("heritrace.routes.merge.get_entity_types")
def test_get_entity_details_sparql_error(
    mock_get_entity_types, mock_get_sparql, app
) -> None:
...
def merge_test_data():
    ...
    return {
        ...
        "resp_agent_uri": URIRef(f"https://orcid.org/{user_orcid}"),
        "entity1_props": {
            "http://merge.test/propA": [
                {"value": "Value A", "type": "literal", "readable_label": "Value A"}
            ],
        },
        "entity2_props": {
            "http://merge.test/propB": [
                {"value": "Value B", "type": "literal", "readable_label": "Value B"}
            ],
        },
    }
...
@patch(
    "heritrace.routes.merge.get_dataset_endpoint",
    return_value="http://db/ds_merge_flash",
)
@patch(
    "heritrace.routes.merge.get_provenance_endpoint",
    return_value="http://db/prov_merge_flash",
)
@patch("heritrace.routes.merge.get_dataset_is_quadstore", return_value=False)
@patch("flask_login.utils._get_user")
def test_execute_merge_success_flash(
    mock_current_user,
    _mock_quadstore,
    _mock_prov,
    _mock_ds,
    ...
    mock_filter_instance.human_readable_entity.side_effect = [
        merge_test_data["entity1_label"],
        merge_test_data["entity2_label"],
    ]
...
@patch("flask_login.utils._get_user")
def test_execute_merge_invalid_primary_source(
    mock_current_user, client, mock_user, merge_test_data
) -> None:
...
@patch(
    "heritrace.routes.merge.get_dataset_endpoint",
    return_value="http://db/ds_merge_save_source",
)
@patch(
    "heritrace.routes.merge.get_provenance_endpoint",
    return_value="http://db/prov_merge_save_source",
)
@patch("heritrace.routes.merge.get_dataset_is_quadstore", return_value=False)
@patch("flask_login.utils._get_user")
...
@patch("flask_login.utils._get_user")
def test_execute_merge_missing_uris(mock_current_user, client, mock_user) -> None:
    ...
    response = client.post(
        url_for("merge.execute_merge"), data={}, follow_redirects=False
    )
...
@patch("heritrace.routes.merge.Editor")
@patch("flask_login.utils._get_user")
def test_execute_merge_editor_value_error(
    mock_current_user,
    mock_editor_cls,
    mock_get_details,
    mock_get_filter,
    ...
    expected_path = urlparse(
        url_for(
            "merge.compare_and_merge",
            subject=merge_test_data["entity1_uri"],
            other_subject=merge_test_data["entity2_uri"],
        )
    ).path
    assert urlparse(response.location).path == expected_path
...
@patch("heritrace.routes.merge.Editor")
@patch("flask_login.utils._get_user")
def test_execute_merge_editor_value_error_flash(
    mock_current_user,
    mock_editor_cls,
    mock_get_details,
    mock_get_filter,
...
@patch("heritrace.routes.merge.Editor")
@patch("flask_login.utils._get_user")
def test_execute_merge_general_exception(
    mock_current_user,
    mock_editor_cls,
    mock_get_details,
    mock_get_filter,
...
@patch("heritrace.routes.merge.Editor")
@patch("flask_login.utils._get_user")
def test_execute_merge_general_exception_flash(
    mock_current_user,
    mock_editor_cls,
    mock_get_details,
    mock_get_filter,
...
@patch("heritrace.routes.merge.get_entity_details")
@patch("flask_login.utils._get_user")
def test_compare_and_merge_success(
    mock_current_user,
    mock_get_details,
    mock_get_filter,
    _mock_sparql,
...
@patch("flask_login.utils._get_user")
def test_compare_and_merge_missing_uris(
    mock_current_user, client, mock_user, merge_test_data
) -> None:
    ...
    response = client.get(
        url_for("merge.compare_and_merge", subject=merge_test_data["entity1_uri"]),
        follow_redirects=False,
    )
...
@patch("flask_login.utils._get_user")
def test_compare_and_merge_fetch_error(
    mock_current_user, _mock_get_details, client, mock_user, merge_test_data
) -> None:
...
@patch("heritrace.routes.merge.get_entity_types")
@patch("flask_login.utils._get_user")
def test_find_similar_resources_success(
    mock_current_user,
    mock_get_types,
    mock_get_sim_props,
    mock_get_filter,
    ...
    mock_sparql_instance.query().convert.side_effect = [
        {
            "results": {
                "bindings": [
                    {
                        "p": {"value": similar_test_data["prop_name"]},
                        "o": {
                            "value": similar_test_data["shared_name"],
                            "type": "literal",
                        },
                    },
                    {
                        "p": {"value": similar_test_data["prop_ref"]},
                        "o": {"value": similar_test_data["shared_ref"], "type": "uri"},
                    },
                ]
            }
        },
        {
            "results": {
                "bindings": [{"similar": {"value": similar_test_data["similar_uri"]}}]
            }
        },
    ]
    mock_get_sparql.return_value = mock_sparql_instance
    ...
    mock_filter_instance = MagicMock()
    mock_filter_instance.human_readable_entity.return_value = similar_test_data[
        "similar_label"
    ]
    mock_filter_instance.human_readable_predicate.side_effect = (
        lambda pred_uri, _entity_key: f"Label_{pred_uri.split('/')[-1]}"
    )
    mock_get_filter.return_value = mock_filter_instance
...
@patch("heritrace.routes.merge.get_similarity_properties")
@patch("flask_login.utils._get_user")
def test_find_similar_resources_no_subject_values(
    mock_current_user,
    mock_get_sim_props,
    mock_get_sparql,
    client,
...
@patch("flask_login.utils._get_user")
def test_find_similar_resources_missing_params(
    mock_current_user, client, mock_user, similar_test_data
) -> None:
    """Test request with missing parameters."""
    mock_current_user.return_value = mock_user
    response = client.get(
        url_for(
            "merge.find_similar_resources", subject_uri=similar_test_data["subject_uri"]
        )
    )
    ...
    mock_sparql_instance.query().convert.side_effect = [
        {
            "results": {
                "bindings": [
                    ...
                    {
                        "p": {"value": prop_plain},
                        "o": {"value": "PlainValue", "type": "literal"},
                    },
                    {
                        "p": {"value": prop_bnode},
                        "o": {"value": "bnode1", "type": "bnode"},
                    },
                ]
            }
        },
...
@patch("heritrace.routes.merge.get_similarity_properties")
@patch("flask_login.utils._get_user")
def test_find_similar_resources_no_valid_conditions(
    mock_current_user,
    mock_get_sim_props,
    mock_get_sparql,
    client,
    ...
    mock_sparql_instance.query().convert.return_value = {
        "results": {
            "bindings": [
                {
                    "p": {"value": prop_bnode_only},
                    "o": {"value": "bnode_val", "type": "bnode"},
                }
            ]
        }
    }
...
@patch("flask_login.utils._get_user")
def test_find_similar_resources_invalid_limit_offset_type(
    mock_current_user, client, mock_user, similar_test_data
) -> None:
...
@patch("flask_login.utils._get_user")
def test_find_similar_resources_invalid_limit_offset_value(
    mock_current_user, client, mock_user, similar_test_data
) -> None:
    ...
    assert json_data_limit["status"] == "error"
    assert (
        "Limit must be positive and offset non-negative" in json_data_limit["message"]
    )
    ...
    assert json_data_offset["status"] == "error"
    assert (
        "Limit must be positive and offset non-negative" in json_data_offset["message"]
    )
...
@patch("heritrace.routes.merge.get_similarity_properties")
@patch("flask_login.utils._get_user")
def test_find_similar_resources_no_similarity_config(
    mock_current_user, mock_get_sim_props, client, mock_user, similar_test_data
) -> None:
    ...
    mock_get_sim_props.assert_called_once_with(
        (similar_test_data["subject_type"], None)
    )


@patch("heritrace.routes.merge.get_similarity_properties")
@patch("flask_login.utils._get_user")
def test_find_similar_resources_empty_derived_properties(
    mock_current_user, mock_get_sim_props, client, mock_user, similar_test_data
) -> None:
...
@patch("heritrace.routes.merge.get_similarity_properties")
@patch("flask_login.utils._get_user")
def test_find_similar_resources_skip_and_group_missing_subject_prop(
    mock_current_user,
    mock_get_sim_props,
    mock_get_sparql,
    client,
    ...
    mock_sparql_instance.query().convert.side_effect = [
        {
            "results": {
                "bindings": [
                    {
                        "p": {"value": prop_a},
                        "o": {"value": "ValueA", "type": "literal"},
                    },
                    {
                        "p": {"value": prop_c},
                        "o": {"value": "ValueC", "type": "literal"},
                    },
                ]
            }
        },
    ...
    assert mock_sparql_instance.setQuery.call_count == 2
    # Verify the generated query does not include the AND block but includes the OR
    # block
...
@patch("heritrace.routes.merge.get_similarity_properties")
@patch("flask_login.utils._get_user")
def test_find_similar_resources_no_valid_union_blocks_due_to_formatting(
    mock_current_user,
    mock_get_sim_props,
    mock_get_sparql,
    client,
...
def test_find_similar_resources_success_with_and_group(
    ...
    mock_sparql_instance.query().convert.side_effect = [
        ...
        {"results": {"bindings": [{"similar": {"value": similar_uri_and}}]}},
    ]
    mock_get_sparql.return_value = mock_sparql_instance
