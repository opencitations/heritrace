    ...
    return {
        ("http://example.org/Person", None): {
            "http://example.org/hasName": [
                {"datatypes": [str(XSD.string)], "min": 1, "max": 1}
            ],
        ...
        ("http://example.org/Address", None): {
            "http://example.org/street": [{"datatypes": [str(XSD.string)], "min": 1}]
        },
    }
...
def test_create_entity_with_shacl(
            ...
            "http://example.org/hasAddress": [
                {
                    ...
                    "properties": {"http://example.org/street": ["123 Main St"]},
                }
            ],
        },
    }
    ...
    response = logged_in_client.post(
        url_for("entity.create_entity"), data={"structured_data": json.dumps(data)}
    )
...
def test_create_entity_without_shacl(
    ...
    data = {
        # Add entity_type since it's required
        "entity_type": "http://example.org/Thing",
        "properties": {
            "http://example.org/hasName": [
    ...
    mock_editor_instance.preexisting_finished.assert_called_once()
    assert (
        mock_editor_instance.create.call_count >= 2
            ...
            "http://example.org/hasAddress": [
                {
                    "entity_type": "http://example.org/Address",
                    "shape": "http://example.org/AddressShape",
                    "properties": {"http://example.org/street": ["First St"]},
                },
                {
                    "entity_type": "http://example.org/Address",
                    "shape": "http://example.org/AddressShape",
                    "properties": {"http://example.org/street": ["Second St"]},
                },
            ]
        },
    }
...
def test_create_entity_with_shape_matching(
    ...
    data = {
        ...
        "properties": {
            "http://example.org/hasAddress": [
                {
                    "entity_type": "http://example.org/Address",
                    # This should match the first field definition
                    "shape": "http://example.org/PersonShape",
                    "properties": {"http://example.org/street": ["123 Home St"]},
                }
            ]
        },
    }
    ...
    data["properties"]["http://example.org/hasAddress"][0]["shape"] = (
        "http://example.org/UnknownShape"
    )
    ...
    )  # Should still succeed as it falls back to first definition
    assert response.json["status"] == "success"
...
def test_create_entity_editor_exception(
    ...
    data = {
        "entity_type": "http://example.org/Person",
        "properties": {"http://example.org/hasName": ["John Doe"]},
    }
...
def test_create_entity_with_direct_uri_reference(
    ...
    form_fields = {
        ("http://example.org/Person", None): {
            "http://example.org/hasAddress": [
                {"nodeShape": "http://example.org/AddressShape", "min": 0}
            ],
            "http://example.org/hasContact": [
                {"objectClass": "http://example.org/Contact", "min": 0}
            ],
        }
    }
    mock_get_form_fields.return_value = form_fields
    ...
    assert any(
        call[0][0] == URIRef("http://example.org/test/123")
        and call[0][1] == URIRef("http://example.org/hasAddress")
        and call[0][2] == URIRef("http://example.org/addresses/existing-address-1")
        for call in create_calls
    )
    assert any(
        call[0][0] == URIRef("http://example.org/test/123")
        and call[0][1] == URIRef("http://example.org/hasAddress")
        and call[0][2] == URIRef("http://example.org/addresses/existing-address-2")
        for call in create_calls
    )
    ...
    assert any(
        call[0][0] == URIRef("http://example.org/test/123")
        and call[0][1] == URIRef("http://example.org/hasContact")
        and call[0][2] == URIRef("http://example.org/contacts/existing-contact")
        for call in create_calls
    )
...
def test_create_entity_with_single_value_properties(
    ...
    form_fields = {
        ("http://example.org/Person", None): {
            ...
            "http://example.org/hasAge": [
                {"datatypes": [str(XSD.integer)], "min": 0, "max": 1}
            ],
            "http://example.org/hasBestFriend": [
                {"objectClass": "http://example.org/Person", "min": 0, "max": 1}
            ],
        }
    }
    mock_get_form_fields.return_value = form_fields
    ...
    data = {
        ...
        "properties": {
            ...
            # Existing entity reference with explicit metadata
            "http://example.org/hasBestFriend": {
                "is_existing_entity": True,
    ...
    assert any(
        call[0][0] == URIRef("http://example.org/test/123")
        and call[0][1] == URIRef("http://example.org/hasName")
        and str(call[0][2]) == "John Doe"
        for call in create_calls
    )
    ...
    assert any(
        call[0][0] == URIRef("http://example.org/test/123")
        and call[0][1] == URIRef("http://example.org/hasAge")
        and str(call[0][2]) == "30"
        for call in create_calls
    )
    ...
    assert any(
        call[0][0] == URIRef("http://example.org/test/123")
        and call[0][1] == URIRef("http://example.org/hasBestFriend")
        and call[0][2] == URIRef("http://example.org/persons/existing-friend")
        for call in create_calls
    )
...
def test_create_entity_invalid_primary_source(
    ...
    mock_gettext.return_value = (
        "Invalid primary source URL provided"  # Mock translation
    )

    data = {
        "properties": {
            "http://example.org/hasName": [
                {"type": "literal", "value": "Invalid Source Test"}
            ]
        }
    }
...
def test_create_entity_save_default_primary_source(
    ...
    data = {
        "entity_type": "http://example.org/Person",
        "properties": {"http://example.org/hasName": ["Test Person"]},
    }
