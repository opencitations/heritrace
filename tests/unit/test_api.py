...
def test_apply_changes_server_error(
    ...
    # Mock the import_entity_graph function to raise an exception that's not a ValueError
...
def test_apply_changes_with_quadstore(
    mock_create_logic, mock_import_entity_graph, api_client: FlaskClient, app: Flask
) -> None:
    """Test the apply_changes endpoint with a quadstore dataset."""
    # Mock the import_entity_graph function to return a mock editor with quadstore support
    mock_editor = MagicMock()
    mock_editor.dataset_is_quadstore = True
    
    # Create a mock Graph object for the graph context
    mock_graph = MagicMock()
    mock_graph.identifier = URIRef("http://example.org/graph/1")
    
    # Set up the mock quad
    mock_quad = (URIRef("http://example.org/entity/1"), URIRef("http://example.org/predicate/1"), 
                Literal("Test Value"), mock_graph)
    
    # Configure the g_set.quads method to return our mock quad
    ...
    # Verify the editor, data, and subject were passed correctly
    assert call_args[0] == mock_editor
    assert call_args[1] == changes[0]["data"]
    assert call_args[2] == changes[0]["subject"]
    
    # For the graph_uri parameter, we need to check that it's the same as our mock graph's identifier
    # The exact mock object might be different, but the identifier should match
