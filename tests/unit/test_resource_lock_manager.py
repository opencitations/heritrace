...
def test_generate_reverse_links_key(resource_lock_manager: ResourceLockManager) -> None:
    """Test the _generate_reverse_links_key method."""
    resource_uri = "http://example.org/resource/123"
    expected_key = "reverse_links:http://example.org/resource/123"
    assert (
        resource_lock_manager.generate_reverse_links_key(resource_uri) == expected_key
    )


def test_get_lock_info_no_lock(
    resource_lock_manager: ResourceLockManager, mock_redis
) -> None:
...
def test_get_lock_info_with_lock(
    resource_lock_manager: ResourceLockManager, mock_redis
) -> None:
...
def test_check_lock_status_available(
    _mock_current_user, resource_lock_manager: ResourceLockManager, mock_redis
) -> None:
...
def test_check_lock_status_reverse_link_locked(
    ...
    resource_lock_manager.generate_reverse_links_key(resource_uri)
    ...
    # Update side_effect to return None for target resource, then lock data for linking
    # resource
...
def test_acquire_lock_success(
    ...
    with patch.object(
        resource_lock_manager,
        "check_lock_status",
        return_value=(LockStatus.AVAILABLE, None),
    ):
        # Call the method
...
def test_release_lock_success(
    ...
    with patch.object(resource_lock_manager, "get_lock_info", return_value=lock_info):
        # Call the method
