...
class TestShaclUtils:
    ...
    def test_shacl_utils_empty_shacl(self) -> None:
    ...
    def test_get_form_fields_from_shacl_early_return(
        self, mock_order, mock_apply, mock_process, mock_extract
    ) -> None:
        """
        Test that get_form_fields_from_shacl returns early when shacl is None without
    ...
    def test_process_query_results_with_ornodes(
        self,
        mock_process_nested_shapes,
        mock_human_readable_class,
        mock_get_object_class,
        mock_get_shape_target_class,
    ) -> None:
        ...
        results = [
            QueryResult(
                ...
                node_shape="nodeShape1",
                has_value=None,
                object_class="objectClass1",
                min_count=0,
                max_count=None,
                datatype=None,
                optional_values="",
                or_nodes="orNode1",  # Questo è importante: specifica un orNode
                condition_path=None,
    ...
    def test_apply_display_rules_to_nested_shapes_parent_prop_not_dict(self) -> None:
        """
        Test che apply_display_rules_to_nested_shapes ritorni nested_fields quando
        parent_prop non è un dizionario.
        """
        # Prepara i dati di test
        nested_fields = [
            {"uri": "http://example.org/predicate1", "displayName": "Original Name"}
        ]
        parent_prop = "not_a_dict"  # Questo non è un dizionario

        result = apply_display_rules_to_nested_shapes(
            nested_fields, parent_prop, "http://example.org/Shape1"
        )
    ...
    @patch("heritrace.utils.filters.Filter.human_readable_class")
    @patch("heritrace.utils.shacl_display.process_nested_shapes")
    def test_process_query_results_update_existing_field(
        self,
        mock_process_nested_shapes,
        mock_human_readable_class,
        mock_get_object_class,
        ...
        """
        Test che process_query_results aggiorni correttamente un campo esistente con
        nuovi datatype e condizioni.
        """
        ...
        # Crea due risultati con lo stesso campo di base ma con datatype e condizioni
        # diverse
        ...
        results = [
            QueryResult(
                ...
                optional_values="",
                or_nodes="",  # Nessun orNode
                condition_path=None,
                condition_value=None,
                pattern=None,
                message=None,
            ),
            # Il secondo risultato ha lo stesso campo base ma con un datatype diverso e
            # una condizione
            QueryResult(
                shape="subjectShape1",
                type="entity_type1",
                ...
                optional_values="",
                or_nodes="",  # Stesso orNodes (vuoto)
                condition_path="conditionPath1",  # Aggiungiamo una condizione
                condition_value="conditionValue1",
        ...
        assert entity_key in result
        assert "predicate1" in result[entity_key]
        # Dovrebbe esserci solo un campo, non due, perché il secondo risultato aggiorna
        # il campo esistente
        assert len(result[entity_key]["predicate1"]) == 1
        ...
        assert (
            len(field_info["conditions"]) == 1
        )  # Solo la condizione non vuota dal secondo risultato
    ...
    def test_validate_new_triple_no_shacl(self) -> None:
        ...
        with (
            ...
            result, old_value, message = validate_new_triple(
                subject, predicate, new_value, "create"
            )
            ...
            result, old_value_returned, message = validate_new_triple(
                subject, predicate, new_value, "update", old_value
            )

            # Verifica che il risultato sia un Literal con lo stesso datatype di
            # old_value
            ...
            result, old_value_returned, message = validate_new_triple(
                subject, predicate, new_value, "create", old_value
            )
    ...
    def test_validate_new_triple_entity_types_not_list(self) -> None:
        """
        Test che validate_new_triple gestisca correttamente entity_types quando non è
        una lista.
        """
        ...
        new_value = "test"
        entity_types = (
            "http://example.org/type1"  # entity_types come stringa, non lista
        )

        # Mock per fetch_data_graph_for_subject e get_shacl_graph
        mock_data_graph = MagicMock()
        # Simula che non ci sono tipi nel grafo dei dati
        mock_data_graph.triples.side_effect = lambda _triple_pattern: []
        ...
        with (
            ...
            with patch(
                "heritrace.utils.shacl_validation.isinstance",
                side_effect=lambda obj, cls: (
                    False if cls is list else isinstance(obj, cls)
                ),
            ):
                validate_new_triple(
                    subject, predicate, new_value, "create", None, entity_types
                )

                # Verifica che la query SHACL sia stata chiamata con entity_types
                # convertito in lista
                # Questo è difficile da testare direttamente, quindi verifichiamo che la
                # funzione non generi errori
                # e che la query SHACL sia stata chiamata
                assert mock_shacl.query.called

    def test_collect_inverse_types(self) -> None:
        ...
        # Aggiungi i tipi delle entità che hanno il soggetto come oggetto (inverse
        # types)
        ...
        assert len(inverse_entities) == 2, (
            "Dovrebbero esserci 2 entità che hanno"
            " il soggetto come oggetto,"
            f" trovate: {inverse_entities}"
        )

        # Verifica che possiamo ottenere i tipi delle entità che hanno il soggetto come
        # oggetto
        entity1_types = list(data_graph.triples((entity1, RDF.type, None)))
        entity2_types = list(data_graph.triples((entity2, RDF.type, None)))
        assert len(entity1_types) == 1, (
            f"Dovrebbe esserci 1 tipo per entity1, trovati: {entity1_types}"
        )
        assert len(entity2_types) == 1, (
            f"Dovrebbe esserci 1 tipo per entity2, trovati: {entity2_types}"
        )
        ...
        inverse_types = []
        for s, _p, _o in data_graph.triples((None, None, URIRef(subject))):
        ...
        assert inverse_type1 in inverse_types, (
            f"inverse_type1 dovrebbe essere in inverse_types: {inverse_types}"
        )
        assert inverse_type2 in inverse_types, (
            f"inverse_type2 dovrebbe essere in inverse_types: {inverse_types}"
        )

    def test_validate_new_triple_inverse_types(self) -> None:
        ...
        inverse_types = []
        for s, _p, _o in data_graph.triples((None, None, subject)):
            o_types = [t[2] for t in data_graph.triples((s, RDF.type, None))]
            inverse_types.extend(o_types)
        ...
        class ShaclGraphWrapper:
            def __init__(self, mock_shacl) -> None:
        ...
        with (
            patch(
                "heritrace.utils.shacl_validation.fetch_data_graph_for_subject",
                return_value=data_graph,
            ),
            patch(
                "heritrace.utils.shacl_validation.get_shacl_graph",
                return_value=shacl_wrapper,
            ),
            patch("heritrace.utils.shacl_validation.len", return_value=1),
            patch(
                "heritrace.utils.shacl_validation.get_custom_filter"
            ...
            patch(
                "heritrace.utils.shacl_validation.get_highest_priority_class",
                return_value="http://example.org/DirectType",
            ),
        ):
            # Configura il mock per custom_filter
            mock_filter = MagicMock()
            ...
            # Verifica che la query contenga VALUES ?type con tutti i tipi (diretto e
            # inversi)
            # Nella query SPARQL, i tipi vengono formattati come <URI>
            assert f"<{direct_type}>" in query_arg, (
                f"Il tipo diretto {direct_type} non è presente nella query"
            )
            ...
            assert f"<{inverse_type1}>" in values_section, (
                f"L'inverse type {inverse_type1} non è presente nella query"
            )
            assert f"<{inverse_type2}>" in values_section, (
                f"L'inverse type {inverse_type2} non è presente nella query"
            )
...
class TestDetermineShapeForEntityTriples:
    ...
    def test_no_shacl_graph(self, _mock_get_priority, mock_get_shacl) -> None:
        """Test when no SHACL graph is available."""
        mock_get_shacl.return_value = None

        triples = [(URIRef("http://example.org/entity1"), RDF.type, FABIO.JournalIssue)]
    ...
    @patch("heritrace.utils.shacl_utils.get_shacl_graph")
    @patch("heritrace.utils.shacl_utils.get_class_priority")
    def test_no_classes_in_triples(self, _mock_get_priority, mock_get_shacl) -> None:
    ...
    @patch("heritrace.utils.shacl_utils.get_shacl_graph")
    @patch("heritrace.utils.shacl_utils.get_class_priority")
    def test_single_candidate_shape(self, _mock_get_priority, mock_get_shacl) -> None:
    ...
    def test_special_issue_vs_issue_distinction(
        self, mock_get_properties, mock_get_priority, mock_get_shacl
    ) -> None:
        """
        Test distinguishing between SpecialIssueShape and IssueShape based on
        properties.
        """
        mock_get_shacl.return_value = self.mock_shacl_graph
        ...
        self.mock_shacl_graph.query.return_value = [
            mock_issue_result,
            mock_special_issue_result,
        ]

        def mock_properties_side_effect(_graph, shape_uri):
            ...
            if shape_uri == "http://schema.org/SpecialIssueShape":
                return {
                    "http://purl.org/spar/fabio/hasSequenceIdentifier",
                    "http://purl.org/vocab/frbr/core#partOf",
    ...
    @patch("heritrace.utils.shacl_utils.get_shacl_graph")
    @patch("heritrace.utils.shacl_utils.get_class_priority")
    def test_no_candidate_shapes(self, _mock_get_priority, mock_get_shacl) -> None:
        ...
        triples = [
            (
                URIRef("http://example.org/entity1"),
                RDF.type,
                URIRef("http://example.org/UnknownClass"),
            )
        ]
    ...
    @patch("heritrace.utils.shacl_utils.get_shacl_graph")
    @patch("heritrace.utils.shacl_utils.get_class_priority")
    def test_with_rdflib_graph_iterator(
        self, _mock_get_priority, mock_get_shacl
    ) -> None:
...
class TestGetShapeProperties:
    ...
    def test_get_shape_properties(self) -> None:
        ...
        result = _get_shape_properties(
            self.mock_shacl_graph, "http://schema.org/TestShape"
        )
    ...
    def test_get_shape_properties_empty(self) -> None:
        """Test when shape has no properties."""
        self.mock_shacl_graph.query.return_value = []

        result = _get_shape_properties(
            self.mock_shacl_graph, "http://schema.org/EmptyShape"
        )
        assert result == set()
...
class TestFindEntityPositionInOrderMap:
    ...
    def test_empty_order_map(self) -> None:
        """Test with empty order map."""
        order_map = {}
        result = _find_entity_position_in_order_map(
            "http://example.org/entity1", order_map
        )
        assert result is None
    ...
    def test_single_element_chain(self) -> None:
        ...
        result = _find_entity_position_in_order_map(
            "http://example.org/entity2", order_map
        )
        assert result is None
    ...
    def test_simple_chain(self) -> None:
        ...
        # Test each position
        assert (
            _find_entity_position_in_order_map("http://example.org/entity1", order_map)
            == 1
        )
        assert (
            _find_entity_position_in_order_map("http://example.org/entity2", order_map)
            == 2
        )
        assert (
            _find_entity_position_in_order_map("http://example.org/entity3", order_map)
            == 3
        )

        # Test non-existent entity
        assert (
            _find_entity_position_in_order_map("http://example.org/entity4", order_map)
            is None
        )
    ...
    def test_multiple_independent_chains(self) -> None:
        ...
        assert (
            _find_entity_position_in_order_map("http://example.org/entity5", order_map)
            == 2
        )

        # Test non-existent entity
        assert (
            _find_entity_position_in_order_map("http://example.org/entity6", order_map)
            is None
        )

    def test_multiple_single_element_chains(self) -> None:
    ...
    def test_complex_multiple_chains(self) -> None:
        ...
        assert (
            _find_entity_position_in_order_map("http://example.org/single", order_map)
            == 1
        )

        # Test two-element chain
        assert (
            _find_entity_position_in_order_map("http://example.org/first", order_map)
            == 1
        )
        assert (
            _find_entity_position_in_order_map("http://example.org/second", order_map)
            == 2
        )

        # Test four-element chain
        assert (
            _find_entity_position_in_order_map("http://example.org/alpha", order_map)
            == 1
        )
        assert (
            _find_entity_position_in_order_map("http://example.org/beta", order_map)
            == 2
        )
        assert (
            _find_entity_position_in_order_map("http://example.org/gamma", order_map)
            == 3
        )
        assert (
            _find_entity_position_in_order_map("http://example.org/delta", order_map)
            == 4
        )

    def test_no_starting_elements(self) -> None:
        """
    ...
    def test_broken_chain_with_missing_element(self) -> None:
    ...
    def test_realistic_ordered_entities(self) -> None:
        """
        ...
        assert (
            _find_entity_position_in_order_map(
                "http://heritrace.org/entity/chapter1", order_map
            )
            == 1
        )
        assert (
            _find_entity_position_in_order_map(
                "http://heritrace.org/entity/chapter2", order_map
            )
            == 2
        )
        assert (
            _find_entity_position_in_order_map(
                "http://heritrace.org/entity/chapter3", order_map
            )
            == 3
        )
        assert (
            _find_entity_position_in_order_map(
                "http://heritrace.org/entity/chapter4", order_map
            )
            == 4
        )
        ...
        assert (
            _find_entity_position_in_order_map(
                "http://heritrace.org/entity/appendixA", order_map
            )
            == 1
        )
        assert (
            _find_entity_position_in_order_map(
                "http://heritrace.org/entity/appendixB", order_map
            )
            == 2
        )
        ...
        assert (
            _find_entity_position_in_order_map(
                "http://heritrace.org/entity/chapter5", order_map
            )
            is None
        )

    def test_order_map_with_none_values_mixed(self) -> None:
        ...
        # First chain
        assert (
            _find_entity_position_in_order_map("http://example.org/start1", order_map)
            == 1
        )
        assert (
            _find_entity_position_in_order_map("http://example.org/middle1", order_map)
            == 2
        )

        # Single element chain
        assert (
            _find_entity_position_in_order_map("http://example.org/start2", order_map)
            == 1
        )

        # Third chain
        assert (
            _find_entity_position_in_order_map("http://example.org/start3", order_map)
            == 1
        )
        assert (
            _find_entity_position_in_order_map("http://example.org/middle3", order_map)
            == 2
        )
        assert (
            _find_entity_position_in_order_map("http://example.org/end3", order_map)
            == 3
        )
...
class TestGetEntityPositionInSequence:
    ...
    def test_with_sparql_query_simple_chain(self, mock_get_sparql) -> None:
    ...
    @patch("heritrace.utils.shacl_utils.get_sparql")
    def test_with_sparql_query_multiple_chains(self, mock_get_sparql) -> None:
    ...
    @patch("heritrace.utils.shacl_utils.get_sparql")
    def test_with_sparql_query_empty_results(self, mock_get_sparql) -> None:
        ...
        result = get_entity_position_in_sequence(
            self.entity_uri, self.subject_uri, self.predicate_uri, self.order_property
        )
        assert result is None

    def test_with_snapshot_simple_chain(self) -> None:
        ...
        snapshot.add(
            (
                URIRef(self.subject_uri),
                URIRef(self.predicate_uri),
                URIRef("http://example.org/entity1"),
            )
        )
        snapshot.add(
    ...
    def test_with_snapshot_multiple_chains(self) -> None:
        ...
        snapshot.add(
            (
                URIRef(self.subject_uri),
                URIRef(self.predicate_uri),
                URIRef("http://example.org/chapter1"),
            )
        )
        snapshot.add(
            (
                URIRef(self.subject_uri),
                URIRef(self.predicate_uri),
                URIRef("http://example.org/chapter2"),
            )
        )
        snapshot.add(
            (
        ...
        snapshot.add(
            (
                URIRef(self.subject_uri),
                URIRef(self.predicate_uri),
                URIRef("http://example.org/appendixA"),
            )
        )

        # Test first chain
        result = get_entity_position_in_sequence(
            "http://example.org/chapter1",
            self.subject_uri,
            self.predicate_uri,
            self.order_property,
            snapshot,
    ...
    @patch("heritrace.utils.shacl_utils.get_sparql")
    def test_with_snapshot_empty_graph(self, mock_get_sparql) -> None:
    ...
    def test_with_empty_snapshot_proper_check(self) -> None:
    ...
    def test_with_snapshot_entity_not_found(self) -> None:
        ...
        snapshot.add(
            (
                URIRef(self.subject_uri),
                URIRef(self.predicate_uri),
                URIRef("http://example.org/other1"),
            )
        )
        snapshot.add(
            (
                URIRef(self.subject_uri),
                URIRef(self.predicate_uri),
                URIRef("http://example.org/other2"),
            )
        )
        snapshot.add(
            (
    ...
    @patch("heritrace.utils.shacl_utils.get_sparql")
    def test_sparql_query_construction(self, mock_get_sparql) -> None:
    ...
    def test_with_snapshot_complex_ordering(self) -> None:
        """
        ...
        assert (
            get_entity_position_in_sequence(
                chapter1, book_uri, has_part, next_part, snapshot
            )
            == 1
        )
        assert (
            get_entity_position_in_sequence(
                chapter2, book_uri, has_part, next_part, snapshot
            )
            == 2
        )
        assert (
            get_entity_position_in_sequence(
                chapter3, book_uri, has_part, next_part, snapshot
            )
            == 3
        )
        ...
        assert (
            get_entity_position_in_sequence(
                chapter4, book_uri, has_part, next_part, snapshot
            )
            is None
        )

    @patch("heritrace.utils.shacl_utils.get_sparql")
    def test_malformed_sparql_results(self, mock_get_sparql) -> None:
    ...
    def test_with_snapshot_circular_reference(self) -> None:
