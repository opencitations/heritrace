...
def test_generate_modification_text_entity_position_caching(
    ...
    with (
        ...
        patch(
            "heritrace.routes.entity.get_predicate_ordering_info",
            return_value="http://example.org/next",
        ),
        patch(
            "heritrace.routes.entity.get_entity_position_in_sequence", return_value=1
        ),
...
def test_generate_modification_text_position_based_sorting(
    ...
    with (
        ...
        patch(
            "heritrace.routes.entity.get_entity_position_in_sequence",
            side_effect=[2, 1],
        ),
        patch("heritrace.routes.entity.is_valid_url", return_value=True),
        patch(
        ...
        # Test that position-based sorting logic was executed (lines 1535-1536,
        # 1559-1560)
        assert "Modifications" in result
        assert "Additions" in result
