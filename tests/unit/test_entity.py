...
def test_validate_entity_data_valid(
    mock_get_custom_filter, mock_get_form_fields
) -> None:
...
def test_validate_entity_data_missing_required(
    ...
    entity_data = {
        "entity_type": "http://example.org/Document",
        "properties": {"http://example.org/description": ["Test Description"]},
    }
...
def test_validate_entity_data_invalid_entity_type(
    ...
    entity_data = {
        ...
        "properties": {"http://example.org/title": ["Test Title"]},
    }
...
def test_validate_entity_data_with_shape(
    ...
    entity_data = {
        ...
        "properties": {
            "http://example.org/hasAddress": [
                {"shape": "residential", "value": "123 Main St"}
            ]
        },
    }
    ...
    entity_data = {
        ...
        "properties": {
            "http://example.org/hasAddress": [
                {"shape": "business", "value": "456 Business Ave"}
            ]
    ...
    assert len(errors) == 1
    assert (
        "residential" in str(errors[0]).lower() or "required" in str(errors[0]).lower()
    )


def test_process_modification_data() -> None:
...
def test_validate_modification_max_count(
    ...
    is_valid, error = validate_modification(
        modification, URIRef("http://example.org/person/123")
    )
    assert not is_valid
...
def test_format_triple_modification_with_relevant_snapshot() -> None:
    ...
    snapshot1.add(
        (URIRef(entity_uri), URIRef("http://example.org/name"), Literal("Old Name"))
    )
    snapshot2.add(
        (URIRef(entity_uri), URIRef("http://example.org/name"), Literal("New Name"))
    )
...
def test_process_modification_data_no_modifications() -> None:
    """Test process_modification_data with no modifications provided."""
    # Create test data with missing modifications
    data = {"subject": "http://example.org/entity/123", "modifications": []}
...
def test_entity_version_timestamp_from_provenance() -> None:
    """
    ...
    with (
        ...
        mock_result = {
            "results": {
                "bindings": [{"generation_time": {"value": "2023-01-01T00:00:00"}}]
            }
        }
        ...
        try:
            ...
            generation_time = mock_result["results"]["bindings"][0]["generation_time"][
                "value"
            ]
...
def test_restore_version_delete_triple() -> None:
...
def test_restore_version_add_quad() -> None:
...
def test_restore_version_deleted_entity() -> None:
    ...
    assert (
        editor.g_set.entity_index[URIRef(entity_uri)]["source"]
        == "http://example.org/source"
    )
...
def test_restore_version_exception(mock_gettext, mock_flash) -> None:
    ...
    # Create test data
    editor = MagicMock()
    editor.save.side_effect = RuntimeError("Test error")
...
def test_find_appropriate_snapshot_skip_deletion() -> None:
    """
    Test find_appropriate_snapshot when snapshot has equal generation and invalidation
    time.
    """
    # Setup test data
    provenance_data = {
...
def test_find_appropriate_snapshot_no_valid_snapshots() -> None:
...
def test_prepare_entity_snapshots_skip_missing_provenance() -> None:
...
def test_prepare_entity_snapshots_skip_no_valid_snapshot() -> None:
...
def test_compute_graph_differences_non_quadstore(mock_get_dataset_is_quadstore) -> None:
    ...
    # Call the function
    triples_to_delete, triples_to_add = compute_graph_differences(
...
def test_determine_object_class_and_shape_valid_uri(
    ...
    graph.add(
        (URIRef(object_uri), URIRef("http://example.org/name"), Literal("John Doe"))
    )
    ...
    mock_is_valid_url.assert_called_once_with(object_uri)
...
def test_format_snapshot_description_simple(
    ...
    mock_filter.human_readable_entity.assert_called_once_with(
        entity_uri,
        (highest_priority_class, "http://example.org/PersonShape"),
        context_snapshot,
    )
...
def test_format_snapshot_description_merge_with_uri(
    ...
    previous_snapshot.add(
        (
            URIRef("http://example.org/person/456"),
            RDF.type,
    ...
    def _human_readable(uri, _entity_key, _snapshot):
        if uri == "http://example.org/person/456":
            return "Jane Doe"
        return "John Doe"
...
def test_format_snapshot_description_merge_invalid_uri(
    ...
    """
    # Setup mocks
    mock_is_valid_url.return_value = False  # Invalid URI
    mock_determine_shape.return_value = "http://example.org/PersonShape"
...
def test_get_deleted_entity_context_info() -> None:
    ...
    snapshot1.add(
        (URIRef(subject), URIRef("http://example.org/name"), Literal("John Doe"))
    )
    ...
    history = {subject: {timestamp1: snapshot1, timestamp2: snapshot2}}
    ...
    with (
        ...
        assert "http://example.org/Person" in call_args
        ...
        context_snapshot, highest_priority_class, entity_shape = (
            get_deleted_entity_context_info(
                is_deleted=is_deleted,
                sorted_timestamps=single_timestamp,
                history=history,
                subject=subject_uri,
            )
        )
...
def test_process_entity_value_nested_entity(
    ...
    predicate = URIRef("http://example.org/hasAuthor")
    value = {
    ...
    mock_create_nested.assert_called_once_with(
        mock_editor, URIRef("http://example.org/nested/123"), value, default_graph_uri
    )
...
def test_process_entity_value_existing_entity(_mock_is_valid_url) -> None:
    ...
    entity_uri = URIRef("http://example.org/entity/456")
    predicate = URIRef("http://example.org/hasAuthor")
    value = {"is_existing_entity": True, "entity_uri": "http://example.org/person/789"}
    default_graph_uri = URIRef("http://example.org/graph")
    matching_field_def = {}
...
def test_process_entity_value_existing_entity_missing_uri() -> None:
...
def test_process_entity_value_uri_literal(
    ...
    # Test data
    entity_uri = URIRef("http://example.org/entity/456")
    predicate = URIRef("http://example.org/hasWebsite")
    ...
    mock_editor.create.assert_called_once_with(
        entity_uri, predicate, URIRef("http://example.org/website"), default_graph_uri
    )
...
def test__process_ordered_entity_value_existing_entity() -> None:
    ...
    mock_editor.create.assert_called_once_with(
        entity_uri, predicate, URIRef(value["entity_uri"]), default_graph_uri
    )
...
def test_process_ordered_properties_single_shape(mock_process_ordered) -> None:
    ...
    ordering_calls = [
        call
        for call in mock_editor.create.call_args_list
        if len(call[0]) == 4 and call[0][1] == ordered_by
    ]
...
def test_process_ordered_properties_multiple_shapes(mock_process_ordered) -> None:
    ...
    # Should have 2 ordering calls: one within PersonShape group, one within OrgShape
    # group
    assert len(ordering_calls) == 2
