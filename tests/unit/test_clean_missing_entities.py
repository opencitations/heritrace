...
def mock_sparql():
    ...
    with patch(
        "heritrace.scripts.clean_missing_entities.SPARQLWrapperWithRetry"
    ) as mock:
...
def test_init(cleaner, mock_sparql) -> None:
...
def test_find_missing_entities(cleaner, mock_sparql) -> None:
    ...
    # Call the method
    missing_entities = cleaner.find_missing_entities_with_references()
...
def test_find_missing_entities_empty(cleaner, mock_sparql) -> None:
...
def test_find_references_to_entity(cleaner, mock_sparql) -> None:
    ...
    # Call the method
    all_refs = cleaner.find_missing_entities_with_references()
...
def test_remove_references(cleaner, mock_sparql, mock_logger) -> None:
    ...
    references = [
        {
            ...
            "predicate": "http://example.org/references",
        },
        {
            "subject": "http://example.org/entity2",
            "predicate": "http://example.org/cites",
        },
    ]
    ...
    # Call the method
    success = cleaner.remove_references(entity_uri, references)
...
def test_find_missing_entities_with_references(cleaner, mock_sparql) -> None:
    ...
    assert {
        ...
        "predicate": "http://example.org/mentions",
    } in refs2
...
def test_find_missing_entities_with_references_empty(cleaner, mock_sparql) -> None:
    """
...
def test_process_missing_entities(cleaner, mock_sparql, mock_logger) -> None:
    """Test process_missing_entities method."""
    with (
        patch.object(
            cleaner, "find_missing_entities_with_references"
        ) as mock_find_missing_with_refs,
        patch.object(cleaner, "remove_references") as mock_remove_refs,
    ):
        ...
        assert (
            results[0]["references"][0]["predicate"] == "http://example.org/references"
        )
...
def test_process_missing_entities_error(cleaner, mock_sparql, mock_logger) -> None:
...
def test_clean_missing_entities_function(mock_cleaner_class) -> None:
    ...
    mock_cleaner_instance.process_missing_entities.return_value = [
        {
            ...
            "references": [{"subject": "s1", "predicate": "p1"}],
            "success": True,
        }
    ]
    ...
    # Verify MissingEntityCleaner was created with the correct endpoint and is_virtuoso
    # flag
    mock_cleaner_class.assert_called_once_with(
        endpoint=endpoint, is_virtuoso=is_virtuoso
    )
...
def test_main_missing_dataset_db_url(
    ...
    def mock_hasattr_func(_obj, name) -> bool:
        if name == "Config":
            return True
        return name != "DATASET_DB_URL"
