...
def test_generate_lock_key(resource_lock_manager: ResourceLockManager) -> None:
    ...
    assert resource_lock_manager.generate_lock_key(resource_uri) == expected_key


def test_get_lock_info_no_lock(resource_lock_manager: ResourceLockManager) -> None:
    """Test get_lock_info when no lock exists."""
    resource_uri = "http://example.org/resource/no-lock"
    lock_key = resource_lock_manager.generate_lock_key(resource_uri)
...
def test_get_lock_info_with_lock(resource_lock_manager: ResourceLockManager) -> None:
...
def test_check_lock_status_available(
    _mock_current_user, resource_lock_manager: ResourceLockManager
) -> None:
...
def test_check_lock_status_exception(
    mock_logger, _mock_current_user, resource_lock_manager: ResourceLockManager
) -> None:
    """Test check_lock_status when an exception occurs."""
    resource_uri = "http://example.org/resource/error"

    # Mock get_lock_info to raise a RedisError
    with patch.object(
        resource_lock_manager, "get_lock_info", side_effect=RedisError("Test exception")
    ):
        status, lock_info = resource_lock_manager.check_lock_status(resource_uri)
...
def test_check_lock_status_reverse_link_locked(
    ...
    """
    Test check_lock_status when a resource that links to the requested resource is
    ...
    target_lock_key = resource_lock_manager.generate_lock_key(resource_uri)
    resource_lock_manager.redis.delete(target_lock_key)

    # Create a lock on the linking resource by another user
    linking_lock_key = resource_lock_manager.generate_lock_key(linking_resource_uri)
    lock_data = {
        "user_id": "other-user",
        "user_name": "Other User",
        "timestamp": datetime.now(timezone.utc).isoformat(),
    ...
    reverse_links_key = resource_lock_manager.generate_reverse_links_key(resource_uri)
...
def test_check_lock_status_reverse_link_locked_by_self(
    ...
    # Verify the result - should be AVAILABLE since the linking resource is locked by
    # the current user
    assert status == LockStatus.AVAILABLE
    assert lock_info is None
...
def test_acquire_lock_exception(
    ...
    # Mock check_lock_status to raise a RedisError
    with patch.object(
...
def test_create_resource_lock_exception(
    ...
    # Mock redis.setex to raise a RedisError
    with patch.object(
        resource_lock_manager.redis, "setex", side_effect=RedisError("Test exception")
    ):
...
def test_release_lock_success(
    ...
    for linked_uri in linked_resources:
        reverse_links_key = resource_lock_manager.generate_reverse_links_key(linked_uri)
    ...
    for linked_uri in linked_resources:
        reverse_links_key = resource_lock_manager.generate_reverse_links_key(linked_uri)
        assert not resource_lock_manager.redis.sismember(
            reverse_links_key, resource_uri
        )
...
def test_decode_redis_item_bytes(resource_lock_manager: ResourceLockManager) -> None:
    """Test _decode_redis_item with bytes input."""
    bytes_item = b"test-bytes"
    result = resource_lock_manager.decode_redis_item(bytes_item)
    assert result == "test-bytes"
    assert isinstance(result, str)


def test_decode_redis_item_string(resource_lock_manager: ResourceLockManager) -> None:
    """Test _decode_redis_item with string input."""
    string_item = "test-string"
    result = resource_lock_manager.decode_redis_item(string_item)
    assert result == "test-string"
    assert isinstance(result, str)


def test_lockinfo_post_init() -> None:
    """
