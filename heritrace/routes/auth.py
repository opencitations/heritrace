...
def callback():
    ...
    orcid = OAuth2Session(current_app.config['ORCID_CLIENT_ID'], state=session['oauth_state'])
    try:
        token = orcid.fetch_token(current_app.config['ORCID_TOKEN_URL'], client_secret=current_app.config['ORCID_CLIENT_SECRET'],
