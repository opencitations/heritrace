...
linked_resources_bp = Blueprint(
    "linked_resources", __name__, url_prefix="/api/linked-resources"
)
                ...
                if prop_config.get("isVirtual"):
                    ...
                    if target_config.get("class") == entity_type:
                        current_app.logger.debug(
                            "Found virtual property target entity type: %s", entity_type
                        )
                        return True
    ...
    for entity_type in entity_types:
        for rule in display_rules:
            for prop in rule.get("displayProperties", []):
                ...
                if (
                    intermediate_config
                    and intermediate_config.get("class") == entity_type
                ):
                    return True, prop["property"]

                for display_rule in prop.get("displayRules", []):
                    nested_intermediate_config = display_rule.get(
                        "intermediateRelation"
                    )
                    if (
                        nested_intermediate_config
                        and nested_intermediate_config.get("class") == entity_type
                    ):
                        return True, prop["property"]
...
def _resolve_proxy_entity(
    subject_uri: str, predicate: str, connecting_predicate: str
) -> tuple[str, str]:
    """
...
def get_paginated_inverse_references(
    subject_uri: str, limit: int, offset: int
...
@login_required
def get_linked_resources_api() -> Response | tuple[Response, int]:
    ...
    if not subject_uri:
        return jsonify(
            {"status": "error", "message": gettext("Missing subject_uri parameter")}
        ), 400
