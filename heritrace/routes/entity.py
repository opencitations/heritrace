...
def about(subject):
    ...
    if history.get(subject):
        ...
        latest_metadata = next(
            (meta for _, meta in provenance[subject].items() 
             if meta['generatedAtTime'] == sorted_timestamps[-1]),
            None
        )

        is_deleted = (latest_metadata and 
        ...
        if is_deleted and len(sorted_timestamps) > 1:
            ...
            subject_classes = [o for _, _, o in context_snapshot.triples(
    ...
    if not is_deleted:
        ...
        if data_graph:
            ...
            (can_be_added, can_be_deleted, datatypes, mandatory_values, 
             optional_values, subject_classes, valid_predicates) = get_valid_predicates(triples)
    ...
    if can_be_added:
        create_form.predicate.choices = [(p, get_custom_filter().human_readable_predicate(p, subject_classes)) for p in can_be_added]
    ...
    return render_template(
        ...
        dataset_db_text_index_enabled=current_app.config['DATASET_DB_TEXT_INDEX_ENABLED'],
...
def entity_version(entity_uri, timestamp):
    ...
    # Get subject classes
    if is_deletion_snapshot and len(sorted_timestamps) > 1:
        subject_classes = [o for _, _, o in context_version.triples(
            (URIRef(entity_uri), RDF.type, None)
        )]
    else:
        subject_classes = [o for _, _, o in version.triples(
            (URIRef(entity_uri), RDF.type, None)
        )]
...
def restore_version(entity_uri, timestamp):
    ...
    try:
        ...
        flash(
            gettext('An error occurred while restoring the version: %(error)s', error=str(e)), 
            'error'
        )
        ...
        for field in predicate_fields:
            ...
            if field.get('nodeShape') or field.get('objectClass'):
                if validators.url(object_value):
                    ...
                    if snapshot:
                        object_classes = [
                            str(o) for s, p, o in snapshot.triples(
...
def validate_modification(modification: dict, subject_uri: str, 
                        form_fields: dict) -> Tuple[bool, str]:
    """
    ...
    if form_fields:
        entity_types = [
            str(t) for t in get_entity_types(subject_uri)
        ]
            ...
            for field in predicate_fields:
                ...
                if operation == 'add':
                    ...
                    if max_count and current_count >= max_count:
                        return False, f'Maximum count exceeded for predicate: {predicate}'
...
def apply_modifications(editor: Editor, modifications: List[dict], 
                       subject_uri: str, graph_uri: Optional[str] = None):
    """
